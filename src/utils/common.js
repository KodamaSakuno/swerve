...
var cBN = (val) => val instanceof BigNumber ? val : new BigNumber(val);
...
export async function ensure_allowance_zap_out(amount, fromContract, toContract, infinite = false) {
    ...
    if(!infinite) {
        ...
        if(allowance.lt(currentContract.max_allowance.div(cBN(2))) && cBN(amount).gt(0)) {
            if(allowance > 0 && requiresResetAllowance.includes(fromContract.address))
...
export async function ensure_allowance(amounts, plain = false, contractName, N_COINS, infinite = false) {
    ...
    for (let i=0; i < N_COINS; i++) {
        calls.push([coins[i].address, coins[i].methods.allowance(default_account, swap).encodeABI()])
    }
    ...
    if (!infinite) {
        ...
        for (let i=0; i < N_COINS; i++) {
            if (cBN(allowances[i]).isLessThan(cBN(amounts[i])) && cBN(amounts[i]).gt(0)) {
                if (allowances[i] > 0 && requiresResetAllowance.includes(coins[i].address))
...
export async function ensure_underlying_allowance(i, _amount, underlying_coins = [], toContract, wrapped = false, contract) {
    ...
    var current_allowance = cBN(await coins[i].methods.allowance(default_account, contract.swap.address).call());
    ...
    if ((current_allowance.isGreaterThan(cBN(0))) & (current_allowance.isLessThan(amount)) && requiresResetAllowance.includes(coins[i].address))
...
export async function approveAmount(contract, amount, account, toContract, infinite = false) {
    ...
    if(!infinite) {
        if(current_allowance.lt(amount) && amount.gt(0)) {
            if(current_allowance > 0 && requiresResetAllowance.includes(contract.address)) {
...
export async function ensure_stake_allowance(amount, stakeContract, infinite = false) {
    ...
    let allowance = cBN(await currentContract.swap_token.methods.allowance(default_account, stakeContract.address).call());
    if(!infinite) {
        if(allowance.lt(amount)) {
            if(allowance.gt(0))
                await approve(currentContract.swap_token, 0, default_account, stakeContract.address)
            await approve(currentContract.swap_token, amount, default_account, stakeContract.address)
        }
    }
    else {
        if(allowance.lt(currentContract.max_allowance.div(cBN(2))) && cBN(amount).gt(0)) {
            if(allowance > 0 && requiresResetAllowance.includes(currentContract.swap_token.address))
                await approve(currentContract.swap_token, 0, default_account, stakeContract.address)
            await approve(currentContract.swap_token, amount, default_account, stakeContract.address)
        }
    }
...
export async function multiInitState(calls, contract, initContracts = false) {
    ...
    // }
    contract.fee = (await contract.swap.methods.fee().call()) / 1e10;
    contract.admin_fee = (await contract.swap.methods.admin_fee().call()) / 1e10;
    var token_balance = await contract.swap_token.methods.balanceOf(default_account).call()
    var token_supply = await contract.swap_token.methods.totalSupply().call()
    ...
    if(initContracts) {
        for (let i = 0; i < 2; i++) {
            contract.coins.push(currentContract.tronWeb.contract(yERC20_abi, allabis[contract.currentContract].coins[i]));
            contract.underlying_coins.push(currentContract.tronWeb.contract(ERC20_abi, allabis[contract.currentContract].underlying_coins[i]));
        }
    ...
    for (let i = 0; i < 2; i++) {
        const balance = await contract.swap.methods.balances(i).call();
...
export async function calc_slippage(values, deposit, zap_values, to_currency) {
    ...
    let calls = [
        [currentContract.swap.address ,currentContract.swap.methods.get_virtual_price().encodeABI()],
    ]
    if(to_currency !== undefined) {
        ...
        calls.push([currentContract.swap.address, currentContract.swap.methods.calc_token_amount(zap_values, to_currency).encodeABI()])
        ...
        calls.push([currentContract.swap.address, currentContract.swap.methods.calc_token_amount(values, deposit).encodeABI()])
    }
    calls.push(...[...Array(currentContract.N_COINS).keys()].map(i => [currentContract.swap.address, currentContract.swap.methods.balances(i).encodeABI()]))
    let aggcalls = await currentContract.multicall.methods.aggregate(calls).call();
